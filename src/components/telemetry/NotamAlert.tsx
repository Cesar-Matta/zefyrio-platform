  ...
  return (
    <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom duration-500">
      <div className="flex items-center gap-2 px-1">
        <AlertTriangle className="w-4 h-4 text-[var(--color-system-orange)]" />
        <h3 className="text-[10px] font-bold tracking-tight text-[var(--z-muted)]">
          NOTAMs Críticos (Área 15nm)
        </h3>
        <span className="ml-auto text-[9px] bg-[var(--color-system-orange)]/10 text-[var(--color-system-orange)] px-2 py-0.5 rounded-full font-bold">
      ...
      {notams.map((notam, i) => (
        <div
          ...
          className="glass-panel p-4 rounded-3xl border border-amber-500/20 bg-[var(--color-system-orange)]/10 group hover:bg-[var(--color-system-orange)]/10 transition-all cursor-pointer">
          <div className="flex items-start gap-4">
            <div className="w-8 h-8 rounded-xl bg-[var(--color-system-orange)]/10 flex items-center justify-center shrink-0">
              <Info className="w-4 h-4 text-[var(--color-system-orange)]" />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] leading-relaxed text-[var(--z-muted)] line-clamp-2 font-medium italic">
                {notam.properties.notamEvent.text}
              </p>
            </div>

            <ChevronRight className="w-4 h-4 text-[var(--z-muted)] group-hover:text-[var(--color-system-orange)] transition-colors mt-3" />
          </div>
        </div>
      ))}
      ...
      {selected && (() => {
        ...
        return (
          <AlertDetailModal
            ...
            icon={Info}
            accent="var(--color-system-orange)"
            badge="NOTAM"
